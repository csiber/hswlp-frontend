...
export const contractStatusTuple = Object.values(CONTRACT_STATUS) as [
  string,
  ...string[],
];

export const contractTable = sqliteTable(
  "contract",
  {
    ...commonColumns,
    id: text()
      .primaryKey()
      .$defaultFn(() => `ctr_${createId()}`)
      .notNull(),
    userId: text()
      .notNull()
...
export const requestTable = sqliteTable(
  ...
  (table) => [index("request_email_idx").on(table.email)],
);
...
export const slowRequestLogTable = sqliteTable(
  "slow_request_log",
  {
    ...commonColumns,
    id: text()
      .primaryKey()
      .$defaultFn(() => `slog_${createId()}`)
      .notNull(),
    url: text().notNull(),
...
export const slowRequestLogRelations = relations(
  slowRequestLogTable,
  ({ one }) => ({
    user: one(userTable, {
